...
bool testSort() {
  ...
  try {
    ...
    inputFileName = ems::findAvailableFileName("testsort_input");
    if (inputFileName.empty()) return false;

    //Find an available output filename
    outputFileName = ems::findAvailableFileName("testsort_output");
    if (outputFileName.empty()) return false;

    //Create the input file
    if (!ems::createRandomFile<key>(inputFileName, 1000, 1000)) {
      cleanup();
      return false;
    }
    ...
    //Check the result
    if (!ems::checkSortedFile<key>(outputFileName)) {
      cleanup();
      return false;
    }
